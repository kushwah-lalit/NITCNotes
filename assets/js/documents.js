...
function showAndFill(name,fileId,requestUserId){
  if(!box){
    box = !box;
    console.log(name,fileId,requestUserId);
...
function togglePublic(link){
  $.ajax({
      type: 'get',
      url: link,
      success: function(data){
        if(data.data.isPublicStatus){
          $('#'+data.data.document_id).replaceWith(`<a id="${data.data.document_id}" onclick="togglePublic('/documents/togglePublic/${data.data.document_id}')" style="color: rgba(214, 214, 214, 1);" data-toggle="tooltip" data-placement="top" title="Make Private"><i class="fa-solid fa-lock-open"></i></a>`);
        }else{
          $('#'+data.data.document_id).replaceWith(`<a id="${data.data.document_id}" onclick="togglePublic('/documents/togglePublic/${data.data.document_id}')" style="color: rgba(214, 214, 214, 1);" data-toggle="tooltip" data-placement="top" title="Make Public"><i class="fa-solid fa-lock"></i></a>`);
        }
        return;
      },error: function(error){
          console.log(error.responseText);
